...
def command_reduce(model_file, target_dim, if_exists):
    ...
    if target_dim >= initial_dim:
        raise Exception(
            "Target dimension (%d) should be less than initial dimension (%d)."
    ...
    if os.path.isfile(result_filename):
        if if_exists == "overwrite":
            ...
            raise Exception("File already exists. Use --overwrite to overwrite.")
...
def main():
    ...
    parser.add_argument("model", type=str, help="model file to reduce. model.bin")
    parser.add_argument("dim", type=int, help="targeted dimension of word vectors.")
    parser.add_argument(
    ...
    command_reduce(
        args.model, args.dim, if_exists=("overwrite" if args.overwrite else "strict")
    )
